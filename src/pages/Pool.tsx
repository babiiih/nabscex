...
import { Plus, Minus, Droplets, TrendingUp, ChevronDown } from "lucide-react";
import { useWallet } from "../contexts/WalletContext";
import { ARC_TOKENS, type Token } from "../config/tokens";
import TokenSelector from "../components/common/TokenSelector";

const MOCK_POOLS = [
  { token0: "USDC", token1: "EURC", tvl: 1250000, apr: 12.5, volume24h: 450000, userLiquidity: 0 },
  { token0: "USDC", token1: "WETH", tvl: 890000, apr: 18.2, volume24h: 320000, userLiquidity: 0 },
  { token0: "USDC", token1: "WBTC", tvl: 2100000, apr: 8.7, volume24h: 780000, userLiquidity: 0 },
  { token0: "EURC", token1: "WETH", tvl: 560000, apr: 15.3, volume24h: 210000, userLiquidity: 0 },
];
...
export default function Pool() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {activeTab === "add" && (
        <div className="max-w-lg mx-auto bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-xl overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            <div>
              <label className="text-xs font-medium text-gray-500 mb-1.5 block">Token A</label>
          ...
          {amountA && amountB && (
            <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-800 space-y-2">
              <div className="flex justify-between text-xs text-gray-500">
                <span>{t("pool.poolShare")}</span><span>0.01%</span>
              </div>
              <div className="flex justify-between text-xs text-gray-500">
                <span>{t("common.rate")}</span><span>1 {tokenA.symbol} = 1.0 {tokenB.symbol}</span>
              </div>
            </div>
          )}
          <div className="p-4">
            {!isConnected ? (
              ...
              <button className="w-full py-3 rounded-2xl bg-gradient-to-r from-brand-500 to-brand-600 text-white font-semibold hover:from-brand-600 hover:to-brand-700 transition-all shadow-lg shadow-brand-500/25">
                {t("pool.addLiquidity")}
              </button>
            )}
          </div>
